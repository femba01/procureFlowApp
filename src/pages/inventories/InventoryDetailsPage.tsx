...
export default function InventoryDetailsPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (value) => {
      client.setQueryData(["inventory-item", itemId], value);
      client.invalidateQueries({ queryKey: ["inventory"] });
      client.invalidateQueries({ queryKey: ["movements", itemId] });
      setOpen(false);
      setReason("");
      setReference("");
    },
    ...
    <>
      ...
      <section className="inventory-hero panel">
        ...
        <div>
          ...
          <p>
            {data.sku} · {data.category} · {data.warehouseName}
          </p>
        </div>
      ...
      <section className="inventory-detail-stats">
        ...
        <article className="panel">
          ...
          <small>After {data.reserved} reserved</small>
        </article>
        <article className="panel">
        ...
        <article className="panel">
          ...
          <strong>{money(data.quantity * data.unitCost)}</strong>
          <small>{money(data.unitCost)} per unit</small>
        </article>
      </section>
        ...
        <aside className="panel reorder-card">
          ...
          <div className="reorder-gauge">
            <div
              style={
                {
                  "--stock-angle": `${Math.min(100, (data.quantity / Math.max(1, data.reorderLevel * 2)) * 100) * 1.8}deg`,
          ...
          <p>
            ...
            <strong>{data.reorderLevel} units</strong>.
          </p>
